...
public class AdaptiveVoiceDetector implements IVoiceDetector {
	...
	public AdaptiveVoiceDetector(int backgroundNoiseFrames, int startOfSpeechVoicedFrameCount,
	                             int startOfSpeechHistoryFrameCount,
	                             int endOfSpeechVoicedFrameCount, int endOfSpeechHistoryFrameCount,
	...
	public boolean isVoiceDetected(short[] frame, int numberOfReadBytes, double signalEnergy) {
		...
		final double[] compareResult =
				computeRecentStats(history, compareSequenceStart, backgroundNoiseFrames);
		...
		final double maxEnergyThreshold =
				(maxEnergy - lowSequenceAverage) * lowSequenceStdevFactor;
		...
		switch (detectorState) {
			case BEFORE:
				...
				if (!lowSequenceChanged) {
					voicedCount = 0;
					for (int i = 0, p = history.size() - 1;
					     i < startOfSpeechHistoryFrameCount && p >= 0;
					     i += 1, p -= 1) {
					...
					final ArrayList<Tuple> considering =
							new ArrayList<>(startOfSpeechHistoryFrameCount);
					voicedCount = 0;
					int index;
